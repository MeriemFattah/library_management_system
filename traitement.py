...
def samples():
    ...
    def update():
        sql1 = """SET FOREIGN_KEY_CHECKS=OFF"""
        sql = """update exemplaire set dat_achat_exemp=%s,Nbr_exemp={},id_util={},amende_forfait={},ref_doc={},type_exemp=%s,etat_exemp=%s where n°exemp = {}""".format(u8.get(),u7.get(),u3.get(),u4.get(),
            u1.get())
        sql2 = """SET FOREIGN_KEY_CHECKS=ON"""
        cursor.execute(sql1)
        cursor.execute(sql,[u2.get(),u5.get(),u6.get()])
        cursor.execute(sql2)
        selected_item1 = t.selection()[0]
        t.item(selected_item1, text="blub", values=(
        u1.get(), u2.get(), u8.get(), u7.get(), u3.get(),u4.get(), u5.get(), u6.get()))
        connection.commit()
    ...
    btn10 = Button(book2, text="Update", bg='#add8e6',
                fg="black", font=('microsft YaHei Light ', 11, 'bold'), command=update)
    btn10.place(x=680, y=320, relwidth=0.25, relheight=0.1)
