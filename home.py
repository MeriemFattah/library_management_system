...
def h ():
    ...
    def insert():
        ...
        mySql_insert_query = '''INSERT INTO aider (id_stag,id_biblio)
    ...
    def Delete():
        ...
        sql = """Delete from aider where id_stag= {}""".format(user1.get())
    ...
    def select():
        cursor.execute(
            """ select * from aider where id_stag={}""".format(user1.get()))
        a = cursor.fetchall()
        for x in a:
            user1.delete(0, END)
    ...
    def update():
        ...
        sql3 = """SET FOREIGN_KEY_CHECKS=1"""
        cursor.execute(sql1)
        sql = """ update aider set id_biblio={} where id_stag={};""".format(user1.get(),user2.get())
        cursor.execute(sql)
        sql3 = """SET FOREIGN_KEY_CHECKS=1"""
        cursor.execute(sql3)
    ...
    tre['columns'] = ('id_intern', 'id_employee')
    tre.column('#0', width=0, stretch=NO)
    tre.column('id_intern', anchor=CENTER, width=80)
    tre.column('id_employee', anchor=CENTER, width=80)

    tre.heading('#0', anchor=CENTER, text='')
    tre.heading('id_intern', anchor=CENTER, text='id_intern')
    tre.heading('id_employee', anchor=CENTER, text='id_employee')
    tre.place(x=70, y=230, relwidth=0.6, relheight=0.4)
    cursor.execute('select * from  aider')
    # fetch result
